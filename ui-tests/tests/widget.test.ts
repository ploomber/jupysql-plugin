...
for (const {
  ...
  connectionName,
  database,
} of relevantFieldsEmbeddedDatabases) {
  test(`test only relevant fields appear in embedded database ${label}`, async ({
    page,
  }) => {
    await displayWidget(page);
...
for (const {
  ...
  password,
  host,
} of fieldDefaultsWhenNoExistingConnection) {
  test(`test field defaults appear if there is no existing connection : ${label}`, async ({
    page,
  }) => {
    await displayWidget(page);
...
for (const { label, connectionName } of aliasDefaultsWithExistingConnection) {
  test(`test default connection alias appears if there is an existing connection : ${label}`, async ({
    page,
  }) => {
    await createNewNotebook(page);
...
for (const {
  ...
  password,
  host,
} of autoPopulatedFieldsEmbeddedDatabases) {
  test(`test only relevant fields are auto-populated in embedded dbs if dropdown selection changes: ${label}`, async ({
    page,
  }) => {
    await displayWidget(page);
...
test("test create new connection shows error if unable to connect", async ({
  page,
}) => {
  await displayWidget(page);
  ...
  await expect(output[0]).toContain(
    "connections.ini: No such file or directory"
  );
});
...
test("test edit connection shows error if unable to connect", async ({
  page,
}) => {
  await createDefaultConnection(page);
  ...
  // check error message
  await expect(page.locator(".user-error-message")).toContainText(
    "duckdb.IOException"
  );
...
test("test error if creates connection with existing name", async ({
  page,
}) => {
  await createDefaultConnection(page);
  ...
  await expect(page.locator(".user-error-message")).toContainText(
    "A connection named 'default' already exists in your connections file"
  );
});
