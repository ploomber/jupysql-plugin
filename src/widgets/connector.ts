...
export class ConnectorView extends DOMWidgetView {
  ...
  drawConnectionsList(connections: Array<Connection>) {
    ...
    connections.forEach((connection: Connection) => {
      ...
      editConnection.onclick = this.handleEditConnectionClick.bind(
        this,
        connection
      );
      ...
      deleteConnection.onclick = this.handleDeleteConnectionClick.bind(
        this,
        connection
      );

      // add buttons to the actions container
      let connectionsButtonsContainer = this.el.querySelector(
        "#connectionsButtonsContainer"
      );
    ...
    select.addEventListener(
      "change",
      this.handleCreateNewConnectionChange.bind(this)
    );

    const newConnectionButton = this.el.querySelector(
      "#createNewConnectionButton"
    );
    newConnectionButton.addEventListener(
      "click",
      this.handleCreateNewConnectionClick.bind(this)
    );
  ...
  handleEditConnectionClick(connection: Connection) {
    this.el.querySelector("#connectionFormHeader").innerHTML =
      "Edit connection";

    // hide connectionsContainer
    (<HTMLElement>(
      this.el.querySelector("#connectionsContainer")
    )).style.display = "none";

    // show newConnectionContainer
    (<HTMLElement>(
      this.el.querySelector("#newConnectionContainer")
    )).style.display = "block";

    const dropdown = <HTMLSelectElement>(
      this.el.querySelector("#selectConnection")
    );
  ...
  handleDeleteConnectionClick(connection: Connection) {
    ...
    cancelButton.addEventListener(
      "click",
      this.hideDeleteMessageApproval.bind(this)
    );
    ...
    deleteButton.addEventListener(
      "click",
      this.deleteConnection.bind(this, connection)
    );
    deleteConnectionMessage.querySelector(".actions").appendChild(deleteButton);

    // hide controllers
    const deleteConnBtn = this.el.querySelector(
      `#deleteConnBtn_${connection["name"].replace(/ /g, "_")}`
    );
  ...
  hideDeleteMessageApproval() {
    ...
    this.el
      .querySelectorAll(".connection-button-actions")
  ...
  drawConnectionDetailsForm(
    ...
    const connectionFormContainer = this.el.querySelector(
    ...
    cancelButton.addEventListener(
      "click",
      this.drawConnectionsList.bind(this, this.connections)
    );
    ...
    if (connectionAlias) {
      ...
      connectionForm.addEventListener(
        "submit",
        this.handleSubmitNewConnection.bind(this)
      );
    } else {
  ...
  handleMessage(content: any) {
    ...
    if (content.method === "check_config_file") {
      ...
      const i = <HTMLElement>(
        this.el.querySelector(".connections-guidelines .no-config-file")
      );
      if (isExist) {
        i.style.display = "none";

        const iKernelMessage = <HTMLElement>(
          this.el.querySelector(".connections-guidelines .no-config-file")
        );
        iKernelMessage.style.display =
          this.connections.length === 0 ? "block" : "none";
      } else {
  ...
  markConnectedButton(connectionName: string) {
    ...
    const selectedButtonEl = <HTMLButtonElement>(
      this.el.querySelector(`#connBtn_${connectionName.replace(/ /g, "_")}`)
    );
  ...
  showErrorMessage(error: string) {
    const errorEl = <HTMLDivElement>(
      this.el.querySelector(".user-error-message")
    );
